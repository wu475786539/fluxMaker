...
// Observe may emit multiple internal synthetic fills per cycle. It consumes
// only a public book snapshot and never has access to a venue order-placement
// client.
func (g *Generator) Observe(instrument config.InstrumentConfig, venueName string, market config.VenueMarketConfig, book domain.Book, now time.Time) (Snapshot, []domain.Fill) {
	...
	if !now.Before(current.nextAt) {
		fills, err := generateFills(instrument, venueName, market, book, now, current.sequence+1)
		current.nextAt = now.Add(randomDuration(cfg.MinIntervalMS, cfg.MaxIntervalMS))
		if err != nil {
			snapshot.Status = "skipped"
			snapshot.Error = err.Error()
		} else {
			current.sequence += uint64(len(fills))
			current.lastGeneratedAt = now
			for i := len(fills) - 1; i >= 0; i-- {
				current.fills = append([]domain.Fill{fills[i]}, current.fills...)
			}
...
func generateFills(instrument config.InstrumentConfig, venueName string, market config.VenueMarketConfig, book domain.Book, now time.Time, baseSequence uint64) ([]domain.Fill, error) {
	cfg := instrument.TradeSimulation
	...
	if firstPrice.Cmp(book.BidPrice) <= 0 || lastPrice.Cmp(book.AskPrice) >= 0 || firstPrice.Cmp(lastPrice) > 0 {
		return nil, fmt.Errorf("no price tick exists strictly inside bid/ask")
	}
	...
	if minQuantity.Cmp(maxQuantity) > 0 {
		return nil, fmt.Errorf("configured quantity range cannot satisfy exchange minimums")
	}

	fills := make([]domain.Fill, 0, batchSize)
	for i := 0; i < batchSize; i++ {
		...
		effectiveMin := minQuantity
		if market.MinNotional.IsPositive() {
			effectiveMin = effectiveMin.Max(market.MinNotional.Div(price).QuantizeUp(market.QuantityStep))
		}
		if effectiveMin.Cmp(maxQuantity) > 0 {
			return nil, fmt.Errorf("configured quantity range cannot satisfy exchange minimums (with min notional)")
		}

		quantity, err := randomStep(effectiveMin, maxQuantity, market.QuantityStep)
		if err != nil {
			return nil, err
		}
